...
const BookDetails: React.FC = () => {
  ...
  if (!book) {
    return (
      <div className="flex flex-col items-center justify-center py-40">
        ...
        <Link to="/browse" className="mt-6 text-[#E6B18A] font-bold hover:text-[#D39A70]">
          Back to Browse
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-fadeIn pb-20">
      <Link
        to="/browse"
        className="inline-flex items-center text-[#CBB8A9] hover:text-[#E6B18A] font-bold transition-colors"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8 space-y-10">
          <div>
            <div className="flex flex-wrap gap-2 mb-8">
              {book.categories.map((cat) => (
                <span
                  key={cat}
                  className="px-4 py-1.5 bg-[#E6B18A]/10 text-[#E6B18A] rounded-xl text-xs font-bold uppercase tracking-widest border border-[#E6B18A]/20"
                >
                  {cat}
                </span>
              ))}
          ...
          <div className="space-y-6">
            ...
            <p className="text-[#CBB8A9] leading-[1.8] text-lg">{book.description}</p>
          </div>
          ...
          <div className="p-10 rounded-[2.5rem] bg-[#1F1511] border border-[#E6B18A]/20 shadow-sm space-y-6 relative overflow-hidden group">
            ...
            {aiSummary ? (
              <div className="relative">
                ...
                <p className="text-[#F5EFEA] leading-[1.8] text-lg italic">"{aiSummary}"</p>
              </div>
            ) : (
              <div className="space-y-6">
                <p className="text-[#CBB8A9] text-lg italic">
                  Curious about the depth of this work? Our AI can distill its core themes for you instantly.
                </p>
                <button
